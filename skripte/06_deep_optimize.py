...
json_config_path = os.path.join(workspace_dir, "skripte", "deep_opt_config.json")
...
def write_set_file(params, filepath):
    ...
    with open(filepath, "w", encoding="utf-16") as f:
        f.write("; deep optimization parameters\n")
        for k, v in params.items():
            f.write(f"{k}={v}\n")

def main():
    # Targeted Search Space (36 combinations)
    onnx_probs = [0.48, 0.50, 0.52]          # 3 values
    grid_steps = [750, 850]                  # 2 values
    lot_multipliers = [1.34, 1.40]           # 2 values
    take_profits = [60, 80, 100]             # 3 values
    ...
    print(f"Generated deep optimization config with {len(runs)} runs.")
    ...
    print("Running deep optimization batch in backtester...")
    ...
    if process.returncode != 0 or not os.path.exists(results_json_path):
        print("Deep optimization batch failed!")
    ...
    for r in runs_results:
        ...
        # Scale = 9.5 / raw_dd (target 9.5% DD)
        if raw_dd > 0:
            scale = 9.5 / raw_dd
            if scale > 1.0:
                scale = 1.0  # Do not scale up to prevent exceeding initial lot sizing rules
        else:
            scale = 1.0
    ...
    print("\n" + "="*120)
    print("TOP 10 DEEP GRID OPTIMIZATION RESULTS - TARGET DRAWDOWN < 10% (RISK SCALED)")
    print("="*120)
    print(f"{'Rank':<4} | {'Run Config':<22} | {'Raw Profit':<10} | {'Raw DD':<8} | {'Trades/Yr':<9} | {'Scale':<6} | {'Scaled Profit':<13} | {'Scaled DD':<9} | {'New PresetFactor':<16}")
    print("-" * 120)
    
    for idx, c in enumerate(valid_candidates[:10]):
        print(f"{idx+1:<4} | {c['name']:<22} | {c['raw_profit']:>10.2f} | {c['raw_dd']:>7.2f}% | {c['trades_per_year']:>9.1f} | {c['scale']:>6.3f} | {c['scaled_profit']:>13.2f} | {c['scaled_dd']:>8.2f}% | {c['new_preset_factor']:>16.1f}")
