...
json_config_path = os.path.join(workspace_dir, "skripte", "tp_opt_config.json")
...
def write_set_file(params, filepath):
    ...
    with open(filepath, "w", encoding="utf-16") as f:
        f.write("; tp optimization parameters\n")
        for k, v in params.items():
            f.write(f"{k}={v}\n")

def main():
    # Targeted search space (36 combinations)
    onnx_probs = [0.50, 0.52]                 # 2 values
    grid_steps = [750, 800, 850]              # 3 values
    lot_multipliers = [1.34, 1.38]            # 2 values
    take_profits = [40, 45, 50]               # 3 values
    ...
    print(f"Generated tp optimization config with {len(runs)} runs.")
    ...
    print("Running tp optimization batch in backtester...")
    ...
    if process.returncode != 0 or not os.path.exists(results_json_path):
        print("TP optimization batch failed!")
    ...
    print("\n" + "="*120)
    print("TOP 10 GRID OPTIMIZATION RESULTS - TARGET DRAWDOWN < 10% (RISK SCALED)")
    print("="*120)
